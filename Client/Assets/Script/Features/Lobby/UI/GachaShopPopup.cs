...
public class GachaShopPopup : UIPopup
{
    ...
    {
        Debug.Log($"��� ���� {gachaCatalog.Banners.Count}");
    ...
    private IEnumerator LoadGachaScnee(Action fadeIn)
    {
        ...
        if (fadeIn != null)
            fadeIn.Invoke();
        _ = AudioManager.Instance.PlayBGM("bgm_gacha");
    }
    ...
    public void Set_Btn(GachaBannerPb data, Sprite bannerImage)
    {
        ...
        BannerBG.sprite = bannerImage; // �̰Ŵ� ���߿� ������. ( ���� �ش� �̹��� ������ 0���� �س��Ҵ�. ) 
        ...
        {
            Debug.Log($"�̱� 1ȸ : {data.Title}");
            StartCoroutine(network.DrawAsync(data.Key, 1, (res) =>
            {
                if (!res.Ok)
                {
                    Debug.LogError($"Draw ����: {res.Message}");
                    return;
                }
