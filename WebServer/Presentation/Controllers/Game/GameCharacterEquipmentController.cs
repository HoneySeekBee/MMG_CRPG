...
namespace Presentation.Controllers.Game
{
    ...
    public class GameCharacterEquipmentController : ControllerBase
    {
        private readonly ICharacterEquipmentService _svc;

        public GameCharacterEquipmentController(ICharacterEquipmentService svc)
        {
            _svc = svc;
        }
        ...
        public async Task<IActionResult> Set(int userId, int characterId, int equipId, [FromBody] SetEquipmentRequest body, CancellationToken ct = default)
        {
            ...
            var cmd = new SetEquipmentCommand(
                ...
                EquipId: equipId,
                InventoryId: body.HasInventoryId ? body.InventoryId : null // 해제면 null
            );
