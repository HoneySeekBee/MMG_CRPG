...
namespace Presentation.Controllers.Game.User
{
    ...
    public sealed class GameMeController : ControllerBase
    {
        private readonly IUserService _users;
        private readonly IUserStageProgressService _progress;
        public GameMeController(IUserService users, IUserStageProgressService progress)
        {
            _users = users;
            _progress = progress;
        }
        ...
        public async Task<ActionResult<UserProfilePb>> Update([FromBody] UpdateProfilePb req, CancellationToken ct)
        {
            try
            {
                var u = await _users.UpdateProfileAsync(
                    CurrentUserId(),
                    new UpdateProfileRequest(req.Nickname, req.IconId == 0 ? null : req.IconId),
                    ct);
        ...
        [HttpGet("stages")]
        public async Task<ActionResult<MyStageProgressListPb>> GetMyStages(CancellationToken ct)
        {
