...
namespace Application.Repositories
{
    ...
    {
        Task<(IReadOnlyList<Domain.Entities.Characters.Character> Items, int TotalCount)>
            GetPagedAsync(int page, int pageSize, int? elementId, int? rarityId, string? search, CancellationToken ct);

        Task<Domain.Entities.Characters.Character?> GetByIdAsync(int id, CancellationToken ct);

        /// <summary>자식 컬렉션(스킬/레벨/승급)까지 포함한 집합을 로드</summary>
        Task<(Domain.Entities.Characters.Character? Character,
        ...
        Task AddAsync(Domain.Entities.Characters.Character entity, CancellationToken ct);
        Task RemoveAsync(Domain.Entities.Characters.Character entity, CancellationToken ct);
