...
namespace Application.Stages
{
    ...
    public static class StageMappings
    {
        public static StageSummaryDto ToSummaryDto(this Stage s) =>
            new(s.Id, s.Chapter, s.Order, s.Name, s.RecommendedPower, s.StaminaCost, s.IsActive);

        public static StageDetailDto ToDetailDto(this Stage s) =>
            new(
                s.Id, s.Chapter, s.Order, s.Name, s.RecommendedPower, s.StaminaCost, s.IsActive,
                s.Waves
                 .OrderBy(w => w.Index)
